	...
	for l in fh :
		...
		if match :
			...
			if not nr[0] :
				...
				aCode = nr[0]
			if not nr[3] :  # normal parses display without extensions input
				print "area code: " + aCode + \
						", exchange: " + nr[1] + ", trunk: " + nr[2]
			else:				 # extensions display 
				print "area code: " + aCode + \
						", exchange: " + nr[1] + ", trunk: " + nr[2] + ", extensions:" + nr[3]
		else :
			print "NO MATCH: " + l
...
def main() :
	...
	#patString = r'(\d{3})\D*(\d{3})\D*(\d{4})\D*(\d{1,4}'
